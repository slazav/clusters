...
class cluster_finder {
  ...
  /// function for getting data; returns true if point should be
  /// included in the cluster; returns false for non-cluster points and
  /// points outside the correct range.
  typedef bool (*check_fn_t)(const Pt & p);
  ...
  std::set<Pt> done; ///< processed points (see below)

  /// find point neighbours 0..7
  static Pt neighbour(const Pt &p, const int dir){
    switch(dir%8){
  ...
  public:
    ...
    cluster_finder(const int x1, const int x2,
                   ...
                   check_fn_t check_fn, dist_fn_t dist_fn):
        x1(x1), x2(x2), y1(y1), y2(y2), x(x1), y(y1),
        check_fn(check_fn), dist_fn(dist_fn) {};
